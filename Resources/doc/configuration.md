...
```yaml
cas_guard:
    # Enter the certificate to identify the CAS server. Set to false if you do not use it. In production, you should use one for security reason.
    ...
    repository:           'App:User' # Example: AppBundle:User
    # property of the repository to compare with provided credentials (username or email)
    property:             username # Example: mail
    route:
